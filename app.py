...
# This now allows requests from your new Vercel URL
CORS(app, resources={
    r"/api/*": {
        "origins": ["http://localhost:3000", "https://med-alert-frontend.vercel.app"]
    },
    r"/health": {
         "origins": "*"
    }
...
# ... (The rest of your Python functions like extract_structured_data and get_ocr_text remain the same) ...
def extract_structured_data(text_to_analyze):
    if not text_to_analyze.strip():
        return None
    prompt = ( "You are an expert data extractor for health products. Analyze the following text extracted from a product's packaging without hallucinating. " "Your task is to identify and extract the following information: the product's name, a brief description (especially noting consumption instructions), and a list of all ingredients. " "Format your response as a JSON object only. The JSON object should have three keys: 'productName', 'description', and 'ingredients'. " "If a piece of information is not found in the text, its value should be null. Do not add any commentary or introductory text outside of the JSON object.\n\n" "Here is the text:\n---\n" f"{text_to_analyze}\n" "---\n\n" "JSON Output:" )
    try:
...
# CORS(app, resources={r"/*": {"origins": "https://med-alert-frontend.vercel.app"}})
...
#     prompt = (
#         "You are an expert data extractor for health products. Analyze the following text extracted from a product's packaging without hallucinating. "
